...
ndata_train = preprocessor.normalise_dataframe(
    df=data_split["df_train"], step=1, standard_norm=True
)
scalers_train = ndata_train["scalers"]
pdata_train = preprocessor.prepare_feature_and_label(
    data_list=ndata_train["normalised_data"]
)
train_data = pdata_train["data"]
X_train = pdata_train["features"]
y_train = pdata_train["labels"]

ndata_test = preprocessor.normalise_dataframe(
    df=data_split["df_test"], step=1, standard_norm=True
)
scalers_test = ndata_test["scalers"]
pdata_test = preprocessor.prepare_feature_and_label(
    data_list=ndata_test["normalised_data"]
)
