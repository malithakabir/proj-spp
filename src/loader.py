...
class DataLoader:
    ...
    def read_local(self, preprocessed=True):
        if preprocessed:
            ...
            self.df_processed = pd.read_csv(self.filename_processed, parse_dates = True, index_col = 0)
        else:
            fname = '{}{}.csv'.format(self.datadir, self.ticker)
            print('Reading from: {}'.format(fname))
            self.filename_raw = fname
            self.df_raw = pd.read_csv(self.filename_raw, header = 0, index_col =  "Date", parse_dates=True)
